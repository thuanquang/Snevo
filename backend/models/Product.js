...
class Product extends BaseModel {
  ...
  async search(filters = {}, pagination = {}) {
    try {
      ...
      if (searchQuery) {
        query = query.or(
          `shoe_name.ilike.%${searchQuery}%,description.ilike.%${searchQuery}%`
        );
      }
  ...
  async findByCategory(categoryId, pagination = {}, sortOptions = {}) {
    try {
      ...
      // Get category info
      const { data: category } = await supabaseConfig
        .getAdminClient()
  ...
  /**
   * 🗑️ SOFT DELETE - Database trigger tự động cascade
   * Chỉ cần soft delete shoe, variants sẽ tự động được cascade delete
   */
  async softDelete(shoeId) {
    try {
      console.log(
        `🗑️ [SOFT DELETE] Starting soft delete for shoe ${shoeId}...`
      );

      // Check if product exists and is active
      const existingProduct = await this.findById(shoeId);
      if (!existingProduct) {
        throw new NotFoundError(`Product with ID ${shoeId} not found`);
      }

      if (!existingProduct.is_active) {
        throw new ValidationError(`Product ${shoeId} is already deleted`);
      }

      // Đếm variants sẽ bị cascade delete
      const { count: activeVariantsCount } = await supabaseConfig
        .getAdminClient()
        .from(constants.DATABASE.TABLES.SHOE_VARIANTS)
        .select("*", { count: "exact", head: true })
        .eq("shoe_id", shoeId)
        .eq("is_active", true);

      console.log(
        `📊 [SOFT DELETE] Found ${activeVariantsCount} active variants to cascade delete`
      );

      // Soft delete shoe
      // ✅ Trigger sẽ TỰ ĐỘNG cascade delete tất cả active variants
      const product = await super.softDelete(shoeId);

      // Verify cascade hoạt động
      const { count: remainingActiveVariants } = await supabaseConfig
        .getAdminClient()
        .from(constants.DATABASE.TABLES.SHOE_VARIANTS)
        .select("*", { count: "exact", head: true })
        .eq("shoe_id", shoeId)
      ...
      console.log(`✅ [SOFT DELETE] Shoe ${shoeId} soft deleted successfully`);
      console.log(
        `✅ [CASCADE] ${cascadedVariants} variants auto-deleted by trigger`
      );
  ...
  /**
   * ♻️ RESTORE - Database trigger tự động cascade restore
   * Chỉ restore variants có CÙNG deleted_at timestamp với shoe
   */
  async restore(shoeId) {
    try {
      console.log(`♻️ [RESTORE] Starting restore for shoe ${shoeId}...`);

      // Check if product exists and is deleted
      const deletedProduct = await this.findById(shoeId, {
        includeDeleted: true,
      });
      if (!deletedProduct) {
        throw new NotFoundError(`Product with ID ${shoeId} not found`);
      ...
      if (deletedProduct.is_active) {
        throw new ValidationError(`Product ${shoeId} is not deleted`);
      }

      // Preview restore bằng helper function
      const { data: previewData } = await supabaseConfig
        .getAdminClient()
        .rpc("preview_restore_variants", { p_shoe_id: shoeId });

      const willRestore =
        previewData?.filter((v) => v.will_restore).length || 0;
      const willSkip = previewData?.filter((v) => !v.will_restore).length || 0;

      console.log(
        `📊 [RESTORE] Preview: ${willRestore} variants will restore, ${willSkip} will skip`
      );

      // Restore shoe
      // ✅ Trigger sẽ TỰ ĐỘNG restore variants có cùng deleted_at timestamp
      const product = await super.restore(shoeId);

      console.log(`✅ [RESTORE] Shoe ${shoeId} restored successfully`);
      console.log(
        `✅ [CASCADE] ${willRestore} variants auto-restored by trigger`
      );
      if (willSkip > 0) {
        console.log(
          `⚠️ [SKIP] ${willSkip} variants NOT restored (deleted separately before)`
        );
      }

      return {
        ...product,
        variants_restored: willRestore,
        variants_skipped: willSkip,
      };
    } catch (error) {
  ...
  /**
   * 📋 Get restore preview
   * Xem trước variants nào sẽ được restore
   */
  async getRestorePreview(shoeId) {
    try {
      ...
      return {
        total_deleted_variants: data?.length || 0,
        will_restore: data?.filter((v) => v.will_restore) || [],
        will_skip: data?.filter((v) => !v.will_restore) || [],
      };
    } catch (error) {
      throw new Error(`Failed to preview restore: ${error.message}`);
    }
  }
  ...
  async getRatingSummary(productId) {
    try {
      const { data: ratingData } = await supabaseConfig
        .getAdminClient()
        .from(constants.DATABASE_TABLES.REVIEWS)
        .select("rating")
        .eq("shoe_id", productId);
      ...
      const ratingCounts = [1, 2, 3, 4, 5].map(
        (rating) => ratings.filter((r) => r.rating === rating).length
      );
      const averageRating =
        ratings.length > 0
  ...
  processProductWithVariants(product) {
    ...
    const colors = [
      ...new Set(variants.map((v) => v.colors?.color_name).filter(Boolean)),
    ];
    const sizes = [
      ...new Set(variants.map((v) => v.sizes?.size_value).filter(Boolean)),
    ];
  ...
  async updateStock(variantId, quantity, operation = "set") {
    try {
      ...
      if (operation === "increment") {
        ...
        updateData = {
          stock_quantity: (variant.stock_quantity || 0) + quantity,
        };
