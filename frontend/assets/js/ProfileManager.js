...
class ProfileManager {
    ...
    async loadAddresses() {
        try {
            ...
            const addressesList = document.getElementById('addressesList');
            if (addressesList) {
                addressesList.innerHTML = '<div class="alert alert-warning">Failed to load addresses. Please try again.</div>';
            }
        }
    }
    ...
    async updateProfileDisplay() {
        ...
        if (profileName) {
            profileName.textContent = this.currentUser.full_name || this.currentUser.username || 'User';
        }
    ...
    async updateAccountSettings() {
        ...
        try {
            if (window.authService && window.authService.supabase) {
                const { data: { user } } = await window.authService.supabase.auth.getUser();
                if (user) {
                    // For Google OAuth, email is automatically verified
                    // For email/password, check user.email_confirmed_at
                    isEmailVerified = user.email_confirmed_at !== null || user.app_metadata?.provider === 'google';
                }
            }
        } catch (error) {
        ...
        if (accountStatus) {
            accountStatus.textContent = isEmailVerified ? 'Active' : 'Pending Verification';
        }
                    ...
                    <div class="card-body">
                        <div class="d-flex justify-content-between align-items-start">
                            ...
                            <div class="address-actions">
                                <button class="btn btn-sm btn-outline-primary me-2" onclick="editAddress(${address.address_id})">
    ...
    async handlePersonalInfoSubmit(e) {
        ...
        try {
            ...
            if (avatarFile) {
                // File selected - use FormData with multipart upload
            ...
            messageDiv.innerHTML = `<i class="fas fa-exclamation-triangle me-2"></i>${error.message || 'Failed to update profile. Please try again.'}`;
            messageDiv.classList.remove('d-none');
     ...
     */
    async handleAddressAdd(formData) {
        const data = Object.fromEntries(formData);
        ...
        try {
            ...
            const { data: { session } } = await window.authService.supabase.auth.getSession();
            if (!session) {
                this.showError('Please log in to add an address');
                return;
            }
            ...
            console.error('Error adding address:', error);
            this.showError(error.message || 'Failed to add address');
        }
    }
     ...
     */
    async handleAddressEdit(addressId, formData) {
        const data = Object.fromEntries(formData);
        ...
        try {
            ...
            const { data: { session } } = await window.authService.supabase.auth.getSession();
            if (!session) {
                this.showError('Please log in to update an address');
                return;
            }
            ...
            console.error('Error updating address:', error);
            this.showError(error.message || 'Failed to update address');
        }
    }
     ...
     */
    async handleAddressDelete(addressId) {
        ...
        try {
            ...
            const { data: { session } } = await window.authService.supabase.auth.getSession();
            if (!session) {
                this.showError('Please log in to delete an address');
                return;
            }
    ...
    async loadSessions() {
        try {
            ...
            const { data: { session } } = await window.authService.supabase.auth.getSession();
            if (!session) {
                sessionsList.innerHTML = '<div class="text-center py-4"><p class="text-muted">No active session found</p></div>';
                return;
            }
            ...
            const sessionsList = document.getElementById('sessionsList');
            if (sessionsList) {
                sessionsList.innerHTML = '<div class="text-center py-4"><p class="text-danger">Failed to load sessions</p></div>';
            }
        }
    }
    ...
    async terminateAllSessions() {
        if (confirm('This will log you out of all devices. You will need to log in again. Continue?')) {
            this.showSuccess('All other sessions have been terminated. You are now logged out.');
    ...
    async executeAccountDeletion() {
        ...
        if (enteredEmail.toLowerCase() !== currentEmail.toLowerCase()) {
            errorDiv.textContent = 'Email address does not match. Please enter the correct email.';
            errorDiv.classList.remove('d-none');
            return;
        }
        
        // Confirm one more time
        if (!confirm('This is your final confirmation. Delete your account permanently?')) {
            return;
        }
        ...
        try {
            ...
            if (response.ok) {
                ...
                const { error } = await window.authService.supabase.auth.admin.deleteUser(
                ...
                // Show success message
                this.showSuccess('Your account has been permanently deleted. You will be logged out shortly.');
