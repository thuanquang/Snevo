    ...
    setupVariantGeneratorListeners() {
        ...
        document.addEventListener('click', (e) => {
            ...
            if (modal && modal.classList.contains('show')) {
                const submitBtn = e.target.closest('#variantGeneratorModal .btn-primary');
...
async openVariantGenerator() {
    ...
    renderColorSelection() {
        ...
        container.innerHTML = this.core.colors.map(color => {
            const hexCode = color.hex_code || color.color_code || '#ccc';
            const hasVariants = this.core.currentVariants.some(v => v.color_id === color.color_id);
            ...
            return `
                <div class="form-check">
                    ...
                    <label class="form-check-label d-flex align-items-center" 
                        ...
                        ${hasVariants ? '<span class="badge bg-info ms-2 small">Has variants</span>' : ''}
                    </label>
                </div>
            `;
        }).join('');
    ...
    renderSizeSelection() {
        ...
        container.innerHTML = this.core.sizes.map(size => {
            const hasVariants = this.core.currentVariants.some(v => v.size_id === size.size_id);
    ...
    updateVariantPreview() {
            ...
            if (totalPossible === 0) {
                ...
                statusEl.innerHTML = `
                    <div class="alert alert-success mb-0">
                        ...
                        <strong>${newCount} new variant${newCount !== 1 ? 's' : ''}</strong> will be created
                        ${existingCount > 0 ? `<br><small>${existingCount} existing variant${existingCount !== 1 ? 's' : ''} will be skipped</small>` : ''}
                    </div>
                `;
            }
        }
        ...
        // Update summary
        document.getElementById("variantGen_Colors").textContent = selectedColors.length;
        document.getElementById("variantGen_Sizes").textContent = selectedSizes.length;
    ...
    async submitGenerateVariants() {
        try {
            ...
            const defaultPrice = parseFloat(
                document.getElementById("variantGen_DefaultPrice").value
            ) || this.core.currentShoe.base_price;
            
            const defaultStock = parseInt(
                document.getElementById("variantGen_DefaultStock").value
            ) || 0;
            ...
            // Confirm
            if (!confirm(`Generate variants for ${totalVariants} combination${totalVariants !== 1 ? 's' : ''}?\n\nNote: Existing variants will be skipped.`)) {
                return;
            }
            ...
            const statusEl = document.getElementById("variantGen_Status");
            if (statusEl) {
                statusEl.innerHTML = '<div class="text-center"><i class="spinner-border spinner-border-sm me-2"></i>Generating variants...</div>';
            }
            ...
            if (response?.success) {
                ...
                if (response.data) {
                    if (typeof response.data === 'object' && 'created_count' in response.data) {
                        created = response.data.created_count || 0;
                        skipped = response.data.skipped_count || 0;
                    } else if (typeof response.data === 'object' && 'created' in response.data) {
                        created = Array.isArray(response.data.created) ? response.data.created.length : 0;
                        skipped = Array.isArray(response.data.skipped) ? response.data.skipped.length : 0;
                ...
                let message = `Successfully generated ${created} new variant${created !== 1 ? 's' : ''}!`;
                if (skipped > 0) {
                    message += `\n${skipped} variant${skipped !== 1 ? 's' : ''} already existed and were skipped.`;
                }
