...
class ReviewManager {
    ...
    setupEventListeners() {
        ...
        const modalElement = document.getElementById('reviewModal');
        if (modalElement) {
            modalElement.addEventListener('hidden.bs.modal', () => this.resetReviewForm());
        }
    }
    ...
    async loadReviewStats() {
        try {
            const stats = await window.reviewsAPI.getProductReviewStats(this.productId);
    ...
    async loadReviews(page = 1) {
        try {
            ...
            const result = await window.reviewsAPI.getProductReviews(this.productId, params);
    ...
    async checkUserReview() {
        try {
            ...
            this.userReview = await window.reviewsAPI.getMyReviewForProduct(this.productId);
    ...
    renderReviewSummary(stats) {
        ...
        for (let i = 5; i >= 1; i--) {
            ...
            const percentage = (stats.percentage_distribution && stats.percentage_distribution[i]) || 0;
            distHTML += `
                        ...
                        ${isOwnReview ? `
                            <div class="review-actions">
                                <button class="btn btn-sm btn-outline-primary me-2" 
                                        onclick="window.reviewManager.editReview(${review.review_id}, ${review.rating}, '${this.escapeHtml(review.comment || '')}')">
                                    <i class="fas fa-edit"></i>
                                </button>
                                <button class="btn btn-sm btn-outline-danger" 
                                        onclick="window.reviewManager.deleteReview(${review.review_id})">
    ...
    renderPagination(result) {
        ...
        const totalPages = result.pagination?.totalPages || result.totalPages || Math.ceil((result.total || 0) / (result.limit || 10));
        ...
        paginationHTML += `
            <li class="page-item ${currentPage === 1 ? 'disabled' : ''}">
                <a class="page-link" href="#" onclick="window.reviewManager.loadReviews(${currentPage - 1}); return false;">
                    Previous
                </a>
            </li>
        `;
    ...
    openReviewModal(reviewId = null, rating = 0, comment = '') {
        ...
        modalTitle.textContent = this.editingReviewId ? 'Edit Your Review' : 'Write a Review';

        // Populate form if editing
        if (this.editingReviewId) {
            this.setStarRating(rating || this.userReview?.rating || 0);
            document.getElementById('reviewComment').value = comment || this.userReview?.comment || '';
